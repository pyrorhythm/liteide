...
GolangHighligher::GolangHighligher(LiteApi::ITextEditor *editor,
                                   QTextDocument *document) : TextEditor::SyntaxHighlighter(document) {
    m_todoList = QString("TODO,BUG,FIXME,NOTE,SECBUG").split(",");
    m_todoRegexp.setPattern(QString(R"(^(%1)([\s\:\(\,]|$))").arg(m_todoList.join("|")));
...
GolangHighligher::~GolangHighligher() = default;

void GolangHighligher::highlightBlock(const QString &text) {
...
void GolangHighligher::highlightBlockHelper(const QString &text) {
    ...
    for (int i = 0; i < tokens.size(); ++i) {
        ...
        if (previousTokenEnd != tk.begin())
            setFormat(previousTokenEnd, tk.begin() - previousTokenEnd,
                      m_creatorFormats[SyntaxHighlighter::VisualWhitespace]);
        ...
        if (tk.is(T_NUMERIC_LITERAL)) {
            ...
            if (tk.is(T_CPP_COMMENT)) {
                ...
                highlightLine(text, startPosition, tk.end() - startPosition,
                              m_creatorFormats[SyntaxHighlighter::Comment]);
            }
            // we need to insert a close comment parenthesis, if
            //  - the line starts in a C Comment (initalState != 0)
            //  - the first token of the line is a T_COMMENT (i == 0 && tk.is(T_COMMENT))
        ...
        } else if (i == 0 && tokens.size() > 1 && tokens.at(0).begin() == 0 && tk.is(T_IDENTIFIER) && tokens.at(1).
                   is(T_COLON)) {
            setFormat(tk.begin(), tk.length(), m_creatorFormats[SyntaxHighlighter::DataType]);
    ...
    if (text.length() > lastTokenEnd)
        highlightLine(text, lastTokenEnd, text.length() - lastTokenEnd,
                      m_creatorFormats[SyntaxHighlighter::VisualWhitespace]);
...
void GolangHighligher::setFoldingIndent(const QTextBlock &block, int indent) {
...
bool GolangHighligher::isPPKeyword(const QStringView &text) {
    switch (text.length()) {
        case 2:
            if (text.startsWith(QLatin1StringView("if")))
                return true;
            break;
...
void GolangHighligher::highlightLine(const QString &text, int position, int length,
...
void GolangHighligher::highlightCommentLine(const QString &text, int position, int length) {
    ...
    while (index != end) {
        ...
        if (isSpace) {
            ...
            if (first) {
                first = false;
                auto match = m_todoRegexp.match(text.mid(start, tokenLength));
                if (match.hasMatch() && match.capturedStart() == 0) {
                    int todoLen = match.captured(1).length();
...
void GolangHighligher::highlightWord(QStringView word, int position, int length) {
