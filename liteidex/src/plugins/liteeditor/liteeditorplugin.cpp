...
LiteEditorPlugin::LiteEditorPlugin() = default;

bool LiteEditorPlugin::load(LiteApi::IApplication *app) {
  m_liteApp = app;
  QString style =
      app->settings()->value(EDITOR_STYLE, "default.xml").toString();
  if (!style.isEmpty()) {
  ...
  auto *factory = new LiteEditorFileFactory(app, this);
  ...
  foreach (QString mime, app->editorManager()->mimeTypeList()) {
    if (mime.startsWith("text/") || mime.startsWith("application/")) {
      ...
      if (imt) {
        QString custom = app->settings()
                             ->value(EDITOR_CUSTOMEXTENSION + mime, "")
                             .toString();
        if (!custom.isEmpty()) {
  ...
  m_toolBarAct->setChecked(
      m_liteApp->settings()->value(EDITOR_TOOLBAR_VISIBLE, true).toBool());
  app->actionManager()->insertViewMenu(LiteApi::ViewMenuToolBarPos,
                                       m_toolBarAct);
  connect(m_toolBarAct, SIGNAL(triggered(bool)), this,
          SLOT(editorToolBarVisibleChanged(bool)));

  m_navBarAct = new QAction(tr("Edit Navigation Bar"), this);
  m_navBarAct->setCheckable(true);
  m_navBarAct->setChecked(
      m_liteApp->settings()->value(EDITOR_NAVBAR_VISIBLE, true).toBool());
  app->actionManager()->insertViewMenu(LiteApi::ViewMenuToolBarPos,
                                       m_navBarAct);
  connect(m_navBarAct, SIGNAL(triggered(bool)), this,
          SLOT(editorNavigateVisibleChanged(bool)));
