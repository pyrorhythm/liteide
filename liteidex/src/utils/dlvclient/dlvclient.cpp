...
bool DlvClient::IsConnect() const { return !m_dlv.isNull(); }

QString DlvClient::Address() const { return m_addr; }
...
bool DlvClient::Connect(const QString &service) {
  ...
  if (!tcpSocket->waitForConnected(5000)) {
    qDebug("could not connect to host at %s:%d",
           serviceUrl.host().toLocal8Bit().data(), servicePort);
    return false;
  }
  m_addr = service;
  m_isCommandBlock = false;
...
void DlvClient::SetCallTimeout(int timeout) { m_callTimeout = timeout; }

int DlvClient::CallTimeout() const { return m_callTimeout; }
...
BreakpointPointer
DlvClient::CreateBreakpointByFuncName(const QString &funcName) const {
  Breakpoint bp;
...
VariablePointer DlvClient::EvalVariable(const EvalScope &scope,
                                        const QString &expr,
                                        const LoadConfig &cfg) const {
  EvalIn in;
  in.Scope = scope;
  in.Expr = expr;
  in.Cfg = cfg;
...
bool DlvClient::SetVariable(const EvalScope &scope, const QString &symbol,
                            const QString &value) {
  SetIn in;
  in.Scope = scope;
  in.Symbol = symbol;
...
QList<Variable> DlvClient::ListPackageVariables(const QString &filter,
                                                const LoadConfig &cfg) const {
...
QList<Variable> DlvClient::ListLocalVariables(const EvalScope &scope,
                                              const LoadConfig &cfg) const {
  ListLocalVarsIn in;
  in.Scope = scope;
  in.Cfg = cfg;
...
QList<Variable> DlvClient::ListFunctionArgs(const EvalScope &scope,
                                            const LoadConfig &cfg) const {
...
QList<Stackframe> DlvClient::Stacktrace(int goroutineId, int depth,
                                        const LoadConfig &cfg) const {
  StacktraceIn in;
  in.Id = goroutineId;
  in.Depth = depth;
...
QList<Location> DlvClient::FindLocation(const EvalScope &scope,
                                        const QString &loc) const {
  FindLocationIn in;
  in.Scope = scope;
  in.Loc = loc;
  FindLocationOut out;
...
QList<AsmInstruction>
DlvClient::DisassembleRange(const EvalScope &scope, quint64 startPC,
                            quint64 endPC, AssemblyFlavour flavour) const {
  DisassembleIn in;
  in.Scope = scope;
  in.StartPC = startPC;
  in.EndPC = endPC;
...
QList<AsmInstruction> DlvClient::DisassemblePC(const EvalScope &scope,
                                               quint64 pc,
                                               AssemblyFlavour flavour) const {
  DisassembleIn in;
  in.Scope = scope;
  in.StartPC = pc;
  in.EndPC = 0;
...
QVariant DlvClient::LastJsonData() const { return m_lastJsonData; }
...
bool DlvClient::callDebuggerCommand(const DebuggerCommand &cmd) {
  ...
  QJsonRpcMessage request = QJsonRpcMessage::createRequest(
      "RPCServer.Command", QJsonValue::fromVariant(param));
...
bool DlvClient::isCommandBlocked() const { return m_isCommandBlock; }
...
bool DlvClient::callBlocked(const QString &method, const JsonDataIn *in,
                            JsonDataOut *out) const {
  QVariantMap param;
  in->toMap(param);
  QJsonRpcMessage request = QJsonRpcMessage::createRequest(
      "RPCServer." + method, QJsonValue::fromVariant(param));

  QJsonRpcMessage response = m_dlv->sendMessageBlocking(request, m_callTimeout);
  if (response.type() == QJsonRpcMessage::Error) {
    // qDebug("error(%d): %s", response.errorCode(),
    // response.errorMessage().toLocal8Bit().data());//
    return false;
  }
