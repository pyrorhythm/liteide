...
Highlighter::Highlighter(QTextDocument *parent)
    : TextEditor::SyntaxHighlighter(parent), m_regionDepth(0),
      m_lastRegionDepth(0), m_indentationBasedFolding(false), m_tabSize(4),
      m_persistentObservableStatesCounter(PersistentsStart),
      m_dynamicContextsCounter(0), m_isBroken(false), m_stringOrComment(false) {
}

Highlighter::~Highlighter() {}

Highlighter::BlockData::BlockData()
    : m_foldingIndentDelta(0), m_originalObservableState(-1) {}

Highlighter::BlockData::~BlockData() {}

void Highlighter::setDefaultContext(
    const QSharedPointer<Context> &defaultContext) {
  m_defaultContext = defaultContext;
  m_persistentObservableStates.insert(m_defaultContext->name(), Default);
  m_indentationBasedFolding =
      defaultContext->definition()->isIndentationBasedFolding();
}
...
void Highlighter::setTabSize(int tabSize) { m_tabSize = tabSize; }
  ...
  if (!m_defaultContext.isNull() && !m_isBroken) {
    try {
      ...
      while (progress.offset() < length)
        iterateThroughRules(text, length, &progress, false,
                            m_currentContext->rules());

      handleContextChange(m_currentContext->lineEndContext(),
                          m_currentContext->definition(), false);
      m_contexts.clear();
      ...
      if (m_indentationBasedFolding) {
        ...
        // In the case region depth has changed since the last time the state
        // was set.
        setCurrentBlockState(
            computeState(extractObservableState(currentBlockState())));
      }
...
void Highlighter::setupDataForBlock(const QString &text) {
  ...
  if (previousBlockState() == -1) {
    ...
    const int observablePreviousState =
        extractObservableState(previousBlockState());
...
void Highlighter::setupFromWillContinue() {
  ...
  if (currentBlockState() == -1 ||
      extractObservableState(currentBlockState()) == Default)
...
void Highlighter::iterateThroughRules(
    const QString &text, const int length, ProgressData *progress,
    const bool childRule, const QList<QSharedPointer<Rule>> &rules) {
  ...
  while (it != endIt && progress->offset() < length) {
    ...
    if (rule->itemData() == "String" || rule->itemData() == "Comment") {
    ...
    if (rule->matchSucceed(text, length, progress)) {
      ...
      if (!m_stringOrComment) {
        if (!rule->beginRegion().isEmpty()) {
          ...
          if (ch == '{' || ch == '(' || ch == '[') {
            blockData(currentBlockUserData())
                ->appendParenthese(
                    Parenthesis(Parenthesis::Opened, ch, startOffset));
          }
        }
        if (!rule->endRegion().isEmpty()) {
          ...
          if (ch == '}' || ch == ')' || ch == ']') {
            blockData(currentBlockUserData())
                ->appendParenthese(
                    Parenthesis(Parenthesis::Closed, ch, startOffset));
          }
        }
      }
      ...
      if (!m_indentationBasedFolding) {
        if (!rule->endRegion().isEmpty()) {
          ...
          if (!currentRegions->isEmpty() &&
              rule->endRegion() == currentRegions->top()) {
            currentRegions->pop();
            --m_regionDepth;
            ...
            //                        if (!m_stringOrComment &&
            //                        progress->isClosingBraceMatchAtNonEnd()) {
        ...
        if (!rule->beginRegion().isEmpty()) {
          blockData(currentBlockUserData())
              ->m_foldingRegions.push(rule->beginRegion());
          ++m_regionDepth;
          //                    if (!m_stringOrComment &&
          //                    progress->isOpeningBraceMatchAtFirstNonSpace())
          //                    {
          //                        ++blockData(currentBlockUserData())->m_foldingIndentDelta;
          //                    }
        }
      ...
      if (progress->isWillContinueLine()) {
        ...
        if (!rule->context().isEmpty() &&
            contextChangeRequired(rule->context())) {
      ...
      // Format is not applied to child rules directly (but relative to the
      // offset of their parent) nor to look ahead rules.
      if (!childRule && !rule->isLookAhead()) {
        if (rule->itemData().isEmpty())
          applyFormat(startOffset, progress->offset() - startOffset,
                      m_currentContext->itemData(),
                      m_currentContext->definition());
        else
          applyFormat(startOffset, progress->offset() - startOffset,
                      rule->itemData(), rule->definition());
      }

      // When there is a match of one child rule the others should be skipped.
      // Otherwise the highlighting would be incorret in a case like
      // 9ULLLULLLUULLULLUL, for example.
  ...
  if (m_lastRegionDepth == m_regionDepth &&
    ...
    if (detlaDeptn ||
        (blockData(currentBlockUserData())->hasParentheses() &&
         (blockData(currentBlockUserData())->parentheses().last().type ==
          Parenthesis::Opened))) {
      blockData(currentBlockUserData())->m_foldingIndentDelta--;
    }
  }
...
void Highlighter::changeContext(
...
void Highlighter::applyFormat(
    int offset, int count, const QString &itemDataName,
    const QSharedPointer<HighlightDefinition> &definition) {
  if (count == 0)
    return;
  ...
  try {
    ...
    // There are some broken files. For instance, the Printf context in java.xml
    // points to an inexistent Printf item data. These cases are considered to
    // have normal text style.
    return;
  }

  blockData(currentBlockUserData())
      ->addSpellCheckZone(offset, itemData->isSpellChecking());
  ...
  //  Please notice that the following are applied every time for item datas
  //  which have customizations. The configureFormats method could be used to
  //  provide a "one time" configuration, but it would probably require to
  //  traverse all item datas from all definitions available/loaded (either to
  //  set the values or for some "notifying" strategy). This is because the
  //  highlighter does not really know on which definition(s) it is working.
  //  Since not many item datas specify customizations I think this approach
...
void Highlighter::createWillContinueBlock() {
  ...
  const int currentObservableState =
...
void Highlighter::mapLeadingSequence(const QString &contextSequence) {
  if (!m_leadingObservableStates.contains(contextSequence))
    m_leadingObservableStates.insert(
        contextSequence, extractObservableState(currentBlockState()));
}

void Highlighter::pushContextSequence(int state) {
  const QVector<QSharedPointer<Context>> &contexts =
      m_persistentContexts.value(state);
...
void Highlighter::pushDynamicContext(
    const QSharedPointer<Context> &baseContext) {
  // A dynamic context is created from another context which serves as its
  // basis. Then, its rules are updated according to the captures from the
  // calling regular expression which triggered the push of the dynamic context.
...
void Highlighter::assignCurrentContext() {
  if (m_contexts.isEmpty()) {
    // This is not supposed to happen. However, there are broken files (for
    // example, php.xml) which will cause this behaviour. In such cases pushing
    // the default context is enough to keep highlighter working.
    m_contexts.push_back(m_defaultContext);
  }
  m_currentContext = m_contexts.back();
}

int Highlighter::extractRegionDepth(const int state) { return state >> 12; }
...
void Highlighter::setFoldIndent(BlockData *data, int indent,
...
void Highlighter::applyIndentationBasedFolding(const QString &text) {
  ...
  // If this line is empty, check its neighbours. They all might be part of the
  // same block.
  if (text.trimmed().isEmpty()) {
    // data->setFoldingIndent(0);
    setFoldIndent(data, 0, currentBlock());
    const int previousIndent =
        neighbouringNonEmptyBlockIndent(currentBlock().previous(), true);
    if (previousIndent > 0) {
      const int nextIndent =
          neighbouringNonEmptyBlockIndent(currentBlock().next(), false);
...
int Highlighter::neighbouringNonEmptyBlockIndent(QTextBlock block,
